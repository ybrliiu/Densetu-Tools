package Densetu::Tools::Web::Controller::UpdateTimeTable::Admin {
  ...
  sub login {
    ...
    if ($pass eq $admin_pass) {
      ...
      if ($check) {
        $self->cookie(admin_password => $pass, {max_age => 1000000, path => $self->url_for('/update_time_table/admin_login_input')});
      }
  ...
  sub download {
    ...
    eval {
      $self->_ftp_session(sub {
        my ($ftp) = @_;
        $ftp->get('player_map_log.dat', $self->config->{app}{update_time_table_path}) || die 'download failed.', $ftp->message;
      });
    };
  ...
  sub upload {
    ...
    eval {
      $self->_ftp_session(sub {
        my ($ftp) = @_;
        $ftp->put($self->config->{app}{update_time_table_path}, 'player_map_log.dat') || die 'put failed.', $ftp->message;
      });
    };
